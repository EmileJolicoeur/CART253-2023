...
function generatingTotalNodes()  {
    ...
    for (let i = 0; i < totalMaxNodes; i++) {   //12
        ...
        let nb  =   i;
        let size    =   128;
        //Setting up node positions on the screen:
        let location    =   (i/2) + 1;
...
function startScreen()  {
    ...
    textAlign(CENTER);
    text	(`Press "Space" to Start`, width/2, 2*height/3);    //in mousePressed() function
    pop();
}
...
function gameScreen()   {
    ...
    fill(255);
    textAlign(CENTER);
    text(`Things to fix:\n -> File with module info\n -> Code NodeCompleted\n -> Code a way to winScreen`, width/2, height/10);
    pop();
}
...
/** The different Key Inputs of the game:   */
function keyPressed()   {
    if (event.keyCode === 32)   {
    ...
    if (gamePhase === `bomb`)   {
        for (let i = 0; i < nodes.length; i++)  {
            let node    =   nodes[i];
            if (node.type === `button`) {
                // if (node.buttonMod.indicator.displayed === true)    {
                //     if (node.buttonMod.indicator.picker)
                // }
            }
        }
    }
